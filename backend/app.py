...
if __name__ == '__main__':
    ...
    if sinhala_loaded:
        ...
        multilang_port = int(os.environ.get("FLASK_MULTILANG_PORT", "5002"))
        print(f"Server running on http://0.0.0.0:{multilang_port}")
        print(f"Access from this computer: http://localhost:{multilang_port}")
        print(f"Access from network: http://YOUR_IP:{multilang_port}")
        print("=" * 50)
        app.run(host="0.0.0.0", port=multilang_port, debug=True)
    else:
        print("=" * 50)

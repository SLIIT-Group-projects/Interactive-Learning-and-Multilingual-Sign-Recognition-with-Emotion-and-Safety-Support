...
import { View, Text, TouchableOpacity, Animated, StyleSheet, ActivityIndicator, Alert, ScrollView, Dimensions, Easing } from 'react-native';
...
const ChildDashboard = ({ navigation }) => {
  ...
  const calculateBadges = () => {
    ...
    if (analytics.totalSessions >= 1) { badgeCount++; badgeName = 'Star Finder'; }
    if (analytics.totalSessions >= 5) { badgeCount++; badgeName = 'Sign Wizard'; }
    if (analytics.totalSessions >= 10) { badgeCount++; badgeName = 'Super Hero'; }
  ...
  const handleLogout = async () => {
    Alert.alert('Bye Bye?', 'Want to go back to the start?', [
      ...
      { text: 'Yes, Bye Bye!', style: 'destructive', onPress: async () => {
          ...
          <Animated.View style={{ opacity: fadeAnim }} className="px-6 pt-4">
            ...
            <View className="flex-row items-center justify-between mb-8">
              <View className="flex-row items-center">
                ...
                >
                  <MaterialCommunityIcons name="auto-fix" size={width > 400 ? 40 : 32} color="#a855f7" />
                </Animated.View>
                <View>
                  <Text className="text-white text-base font-bold opacity-90">Hi there,</Text>
                  <Text className="text-white text-2xl sm:text-3xl font-black">{childName}! 🦄</Text>
                </View>
              </View>
              <TouchableOpacity
            ...
            <View className="bg-white rounded-[45px] p-8 mb-8 shadow-2xl shadow-purple-200 border-b-8 border-purple-100">
              <View className="flex-row items-center justify-between mb-6">
                <View className="flex-row items-center">
                  <MaterialCommunityIcons name="auto-fix" size={30} color="#a855f7" />
                  <Text className="text-purple-900 text-xl sm:text-2xl font-black ml-2">My Magic Stats</Text>
                </View>
                <View className="bg-purple-100 px-4 py-2 rounded-full">
                  <Text className="text-purple-600 font-black text-sm">{progressPercentage.toFixed(0)}%</Text>
                </View>
              </View>
              ...
              <View className="flex-row justify-between">
                <View className="items-center">
                  <View className="w-12 h-12 bg-purple-100 rounded-2xl items-center justify-center mb-2">
                    <MaterialIcons name="auto-awesome" size={24} color="#a855f7" />
                  </View>
                  <Text className="text-purple-400 text-[10px] font-black uppercase">Signs</Text>
                  <Text className="text-purple-900 text-lg font-black">{lettersLearned}</Text>
                </View>
                <View className="items-center">
                  ...
                  <Text className="text-pink-400 text-[10px] font-black uppercase">Stars</Text>
                  <Text className="text-pink-900 text-lg font-black">{starsEarned}</Text>
                </View>
                <View className="items-center">
                  <View className="w-12 h-12 bg-violet-100 rounded-2xl items-center justify-center mb-2">
                    <MaterialCommunityIcons name="wizard-hat" size={24} color="#8b5cf6" />
                  </View>
                  <Text className="text-violet-400 text-[10px] font-black uppercase">Rank</Text>
                  <Text className="text-violet-900 text-xs font-black">{badges.name}</Text>
                </View>
              </View>
            </View>
            ...
            <View className="flex-row flex-wrap justify-between mb-8">
              ...
              >
                ...
                <Text className="text-purple-900 font-black text-lg">Learn</Text>
                <Text className="text-purple-600 text-xs font-bold">New Magic!</Text>
              </TouchableOpacity>
              ...
              >
                <View className="w-14 h-14 bg-white rounded-3xl items-center justify-center mb-4 shadow-md">
                  <MaterialIcons name="videogame-asset" size={32} color="#8b5cf6" />
                </View>
                <Text className="text-violet-900 font-black text-lg">Games</Text>
                <Text className="text-violet-600 text-xs font-bold">Play time!</Text>
              </TouchableOpacity>
              ...
              >
                ...
                <Text className="text-fuchsia-900 font-black text-lg">Stories</Text>
                <Text className="text-fuchsia-600 text-xs font-bold">Tales!</Text>
              </TouchableOpacity>
              ...
              >
                <View className="w-14 h-14 bg-white rounded-3xl items-center justify-center mb-4 shadow-md">
                  <MaterialCommunityIcons name="shield-star" size={32} color="#ec4899" />
                </View>
                <Text className="text-pink-900 font-black text-lg">Safe</Text>
                <Text className="text-pink-600 text-xs font-bold">I'm Secure!</Text>
              </TouchableOpacity>
            ...
            <View className="bg-white rounded-[45px] p-8 shadow-2xl shadow-purple-200 border-b-8 border-purple-100 items-center overflow-hidden">
              ...
              <View className="bg-purple-600 px-6 py-2 rounded-full">
                <Text className="text-white font-bold text-sm">Magic Accuracy ✨</Text>
              </View>
              <Text className="text-purple-300 text-[10px] mt-4 text-center px-6">
                You're getting better every day! Keep practicing to increase your magic power.
              </Text>
            </View>

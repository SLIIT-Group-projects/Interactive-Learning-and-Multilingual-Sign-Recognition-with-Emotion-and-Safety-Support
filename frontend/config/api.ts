...
export function getBaseUrl(): string {
  ...
  if (__DEV__) {
    ...
    if (isExpoGo) {
      const deviceUrl = process.env.EXPO_PUBLIC_API_URL || "http://192.168.13.67:5000"; // ✅ Updated to match backend port 5000
    ...
    if (Platform.OS === "android") {
      if (isDevice) {
        ...
        const possibleIPs = ["192.168.13.67", "192.168.13.67"]; // ✅ Your IP: 192.168.13.67
      ...
      if (isRealDevice) {
        ...
        return "http://192.168.13.67:5000"; // ✅ Updated to match backend port 5000
      } else {
